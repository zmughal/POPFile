...
!endif
  ...
  ;
  ; (these two constants do not follow the 'CBP_' naming convention described below)
  ;--------------------------------------------------------------------------
...
!ifdef CBP.nsh_included
  !error "${MB_NL}${MB_NL}Fatal error: CBP.nsh has been included more than once!${MB_NL}"
!else
...
ok_to_create_bucket:
  ...
  FileWrite ${CBP_L_FILE_HANDLE} "__CORPUS__ __VERSION__ 1${MB_NL}"
  FileClose ${CBP_L_FILE_HANDLE}
  IfErrors  incrm_ptr
  ...
  FileWrite ${CBP_L_FILE_HANDLE} "${CBP_L_COLOR}${MB_NL}"
  FileClose ${CBP_L_FILE_HANDLE}
...
#----------------------------------------------------------------------------------------------
  ...
  FileWrite ${L_INI_HANDLE} \
    ...
    #${MB_NL}\
    # Copyright (c) 2003-2007 John Graham-Cumming${MB_NL}\
    #${MB_NL}\
    ...
    #   but WITHOUT ANY WARRANTY; without even the implied warranty of${MB_NL}\
    #   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the${MB_NL}\
    #   GNU General Public License for more details.${MB_NL}\
    #${MB_NL}\
    #   You should have received a copy of the GNU General Public License${MB_NL}\
    #   along with POPFile; if not, write to the Free Software${MB_NL}\
    #   Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA${MB_NL}\
    #${MB_NL}\
    #-------------------------------------------------------------------${MB_NL}\
    ${MB_NL}"
  ...
  !insertmacro CBP_WRITE_INI "Settings" "RTL" "$(^RTL)"
...
Function CBP_StrCheckName
  ...
  !define L_BAD_OFFSET    $1   ; Number of characters in ${CBP_VALIDCHARS} string
  !define L_RESULT        $2   ; Holds the result (either "" or a valid bucket name derived from the input string)
  !define L_CURRENT       $3   ; A character from the input string
  !define L_OFFSET        $4   ; The offset to a character in the "validity check" string
  !define L_VALIDCHAR     $5   ; A character from the "validity check" string
  !define L_VALIDLIST     $6   ; Holds the current "validity check" string
  !define L_CHARSLEFT     $7   ; To cater for MBCS input strings, terminate when end of string reached, not when a null byte reached
  ...
  StrLen ${L_BAD_OFFSET} "${CBP_VALIDCHARS}"
  StrCpy ${L_RESULT} ""
...
next_input_char:
  ...
  StrCpy ${L_VALIDLIST} ${CBP_VALIDCHARS}${L_CURRENT}  ; Add it to end of "validity check" to guarantee a match
...
next_valid_char:
  ...
  IntCmp ${L_OFFSET} ${L_BAD_OFFSET} bad_name 0 bad_name ; If match is with the char we added, name is bad
  StrCpy ${L_RESULT} ${L_RESULT}${L_VALIDCHAR}           ; Use "valid" character to ensure we store lowercase letters in the result
...
bad_name:
  ...
  StrCpy ${L_STRING} ${L_RESULT}  ; Result is either a valid bucket name or ""
  Pop ${L_CHARSLEFT}
  Pop ${L_VALIDLIST}
  Pop ${L_VALIDCHAR}
  Pop ${L_OFFSET}
...
#
# Bucket names are assumed to be valid, i.e. they are assumed to only contain characters in
# the ranges 'a' to 'z' and '0' to '9', plus the '-' and '_' characters.
#
# The 'CBP_MakePOPFileBuckets' function uses this function to extract a bucket color from
# a list of default color names (these names only use characters in the range 'a' to 'z').
#----------------------------------------------------------------------------------------------
# Inputs:
